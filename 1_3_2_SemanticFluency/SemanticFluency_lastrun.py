...
expName = 'SemanticFluency'  # from the Builder filename that created this script
expInfo = {
    '编号': '',
...
def setupData(expInfo, dataDir=None):
    ...
    if dataDir is None:
        dataDir = _thisDir
    filename = u'data/1_3_2_%s_%s_%s' % (expName, expInfo['编号'], expInfo['date'])
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/chen-nanwu/Desktop/CEST_1128/1_3_2_SemanticFluency/SemanticFluency_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # Make folder to store recordings from mic_3
    mic_3RecFolder = filename + '_mic_3_recorded'
    if not os.path.isdir(mic_3RecFolder):
        os.mkdir(mic_3RecFolder)
    ...
    IntroComponents = [exp_name, intro1, key_resp]
    ...
    while continueRoutine:
        ...
        # if exp_name is starting this frame...
        if exp_name.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            exp_name.frameNStart = frameN  # exact frame index
            exp_name.tStart = t  # local t and not account for scr refresh
            exp_name.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(exp_name, 'tStartRefresh')  # time at next scr refresh
            # update status
            exp_name.status = STARTED
            exp_name.setAutoDraw(True)
        
        # if exp_name is active this frame...
        if exp_name.status == STARTED:
            # update params
            pass
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('ready1.started', globalClock.getTime())
    key_resp_2.keys = []
    key_resp_2.rt = []
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    ready1Components = [text, text_4, key_resp_2]
    ...
    for thisComponent in ready1Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('ready1.stopped', globalClock.getTime())
    # check responses
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('mic.started', globalClock.getTime())
    # Run 'Begin Routine' code from code_2
    msg=""
    # keep track of which components have finished
    micComponents = [mic_2, text_3]
    ...
    while continueRoutine and routineTimer.getTime() < 60.0:
        ...
        # if mic_2 is starting this frame...
        ...
        # if mic_2 is stopping this frame...
        if mic_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > mic_2.tStartRefresh + 60-frameTolerance:
    ...
    for thisComponent in micComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('mic.stopped', globalClock.getTime())
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('ready2.started', globalClock.getTime())
    key_resp_3.keys = []
    key_resp_3.rt = []
    _key_resp_3_allKeys = []
    # keep track of which components have finished
    ready2Components = [text_2, text_5, key_resp_3]
    ...
    for thisComponent in ready2Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('ready2.stopped', globalClock.getTime())
    # check responses
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('mic2.started', globalClock.getTime())
    # Run 'Begin Routine' code from code_4
    msg=""
    # keep track of which components have finished
    mic2Components = [mic_3, text_6]
    for thisComponent in mic2Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine and routineTimer.getTime() < 60.0:
        ...
        # if mic_3 is starting this frame...
        if mic_3.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            mic_3.frameNStart = frameN  # exact frame index
            mic_3.tStart = t  # local t and not account for scr refresh
            mic_3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(mic_3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('mic_3.started', t)
            # update status
            mic_3.status = STARTED
            # start recording with mic_3
            mic_3.start()
        
        # if mic_3 is active this frame...
        if mic_3.status == STARTED:
            # update params
            pass
            # update recorded clip for mic_3
            mic_3.poll()
        
        # if mic_3 is stopping this frame...
        if mic_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > mic_3.tStartRefresh + 60-frameTolerance:
                # keep track of stop time/frame for later
                mic_3.tStop = t  # not accounting for scr refresh
                mic_3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('mic_3.stopped', t)
                # update status
                mic_3.status = FINISHED
                # stop recording with mic_3
                mic_3.stop()
    ...
    for thisComponent in mic2Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('mic2.stopped', globalClock.getTime())
    # tell mic to keep hold of current recording in mic_3.clips and transcript (if applicable) in mic_3.scripts
    # this will also update mic_3.lastClip and mic_3.lastScript
    mic_3.stop()
    tag = data.utils.getDateStr()
    mic_3Clip = mic_3.bank(
        tag=tag, transcribe='None',
        config=None
    )
    thisExp.addData('mic_3.clip', os.path.join(mic_3RecFolder, 'recording_mic_3_%s.wav' % tag))
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if routineForceEnded:
        routineTimer.reset()
    else:
    ...
    # save mic_3 recordings
    for tag in mic_3.clips:
        for i, clip in enumerate(mic_3.clips[tag]):
            clipFilename = 'recording_mic_3_%s.wav' % tag
            # if there's more than 1 clip with this tag, append a counter for all beyond the first
            if i > 0:
                clipFilename += '_%s' % i
            clip.save(os.path.join(mic_3RecFolder, clipFilename))
