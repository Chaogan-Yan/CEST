...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'audio/innstr_2.wav', 'path': 'audio/innstr_2.wav'},
...
function prac_TrialRoutineBegin(snapshot) {
  return async function () {
    ...
    prac_TrialComponents.push(prac_fixation_star);
...
function prac_TrialRoutineEnd(snapshot) {
  return async function () {
    ...
    if ((prac_response.corr === 1)) {
        prac_feedb = `正确!RT=${prac_response.rt.toFixed(2)}秒`;
...
function prac_Block_FeedbackRoutineBegin(snapshot) {
  return async function () {
    ...
    prac_Block_FeedbackComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function TrialRoutineBegin(snapshot) {
  return async function () {
    ...
    // keep track of which components have finished
    TrialComponents = [];
    TrialComponents.push(fixation_star);
...
function Block_FeedbackRoutineBegin(snapshot) {
  return async function () {
    ...
    Block_FeedbackComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
