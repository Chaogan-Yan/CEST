...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'trials_loop.xlsx', 'path': 'trials_loop.xlsx'},
    {'name': 'trials_loop.xlsx', 'path': 'trials_loop.xlsx'},
    {'name': 'same示例.jpg', 'path': 'same示例.jpg'},
    {'name': 'different示例.jpg', 'path': 'different示例.jpg'},
  ]
});
...
async function updateInfo() {
  ...
  psychoJS.experiment.dataFileName = (("." + "/") + `data/1_5_1_${expName}_${expInfo["\u7f16\u53f7"]}_${expInfo["date"]}`);
...
async function experimentInit() {
  ...
  same_e_g = new visual.ImageStim({
    ...
    image : 'same示例.jpg', mask : undefined,
    anchor : 'center',
  ...
  different_e_g = new visual.ImageStim({
    ...
    image : 'different示例.jpg', mask : undefined,
    anchor : 'center',
    ori : 0.0, pos : [0, 0], size : [0.6, 0.4],
    color : new util.Color([1,1,1]), opacity : undefined,
    flipHoriz : false, flipVert : false,
...
function instr_2RoutineEachFrame() {
  return async function () {
    ...
    if (instr_key_resp_2.status === PsychoJS.Status.STARTED) {
      let theseKeys = instr_key_resp_2.getKeys({keyList: ['f'], waitRelease: false});
...
function instr_3RoutineEachFrame() {
  return async function () {
    ...
    if (instr_key_resp_3.status === PsychoJS.Status.STARTED) {
      let theseKeys = instr_key_resp_3.getKeys({keyList: ['j'], waitRelease: false});
...
function prac_expRoutineEachFrame() {
  return async function () {
    ...
    if (prac_key_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = prac_key_resp.getKeys({keyList: ['f', 'j'], waitRelease: false});
...
function prac_trial_fbRoutineBegin(snapshot) {
  return async function () {
    ...
    prac_feedb_text.setColor(new util.Color(prac_feedbColor));
