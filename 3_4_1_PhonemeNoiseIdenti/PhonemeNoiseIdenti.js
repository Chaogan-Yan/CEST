...
async function updateInfo() {
  ...
  psychoJS.experiment.dataFileName = (("." + "/") + `data/3_4_1_${expName}_${expInfo["\u7f16\u53f7"]}_${expInfo["date"]}`);
...
function experi_practiceLoopBegin(experi_practiceLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisExperi_practice of experi_practice) {
...
function experi_realLoopBegin(experi_realLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisExperi_real of experi_real) {
...
function phoneme_practiceRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of phoneme_practiceComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function response_key_practiceRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of response_key_practiceComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function reponse_accRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of reponse_accComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function response_feedback_judgementRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of response_feedback_judgementComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function intro_realRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of intro_realComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function phoneme_startRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of phoneme_startComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function response_keyRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of response_keyComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function exprement_goodbyeRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of exprement_goodbyeComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
